...
basic_atk = 322  # 攻击力
aux = 32      # 辅助力
critic_rate = 0.05 # 暴击率
critic_multiplier = 150    # 暴击倍率
skill_multiplier = 10500  # 技能倍率
...
def find_max_damage(current_combination, remaining_increments):
    ...
    if not remaining_increments:
        ...
        aux_current = aux + 33.34 * TT_aux + 75 + 30 # 带鹤子T卡，满命额外30
        critic_rate_current = critic_rate + 0.32 + 0.1 + 0.15 * TT_cri_r # 全员到齐增加32%暴击率，月全队buff10%暴击率
        critic_rate_current = min(critic_rate_current, 1) #暴击率超过1时取1
        critic_multiplier_current = critic_multiplier + 50 * TT_cri_m
        skill_multiplier_current = (skill_multiplier + 1500 * TT_skill) * (1 + 0.03 * aux_current)
